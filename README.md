...
本仓库为云发鹏毕业设计项目，主题为基于电子对抗网络的空地协同部署优化。项目实现了基于区域分解和多目标粒子群优化（MOPSO-DT）的雷达/电子对抗节点部署优化算法，同时围绕有效覆盖率（ECR）和最小等效干扰强度开展实验评估。

核心源码集中在 `src/`，实验入口集中在 `experiments/`，结构化结果位于 `results/`，论文主源位于 `TongjiThesis-1.4.3/`。
